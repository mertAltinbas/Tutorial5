...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnEmployeeManagerPairs()
    {
        ...
        var result = emps.Where(e => e.Mgr != null)
            .Join(
                ...
                e1 => e1.Mgr,
                e2 => e2.EmpNo,
                (emp, mngr) => new { Employee = emp.EName, Manager = mngr.EName }
            )
            .ToList();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps.Join(
            ...
            grades,
            ed => (ed.Emp.Sal >= grades.Min(g => g.Losal) && ed.Emp.Sal <= grades.Max(g => g.Hisal)),
            g => true,
            (ed, g) => new
            {
                EName = ed.Emp.EName,
                DName = ed.Dept.DName,
                Grade = grades.First(gr => ed.Emp.Sal >= gr.Losal && ed.Emp.Sal <= gr.Hisal).Grade
            }
        ).ToList();
